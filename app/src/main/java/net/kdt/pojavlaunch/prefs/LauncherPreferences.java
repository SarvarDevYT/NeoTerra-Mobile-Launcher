...
public class LauncherPreferences {
    ...
    public static void loadPreferences(Context ctx) {
        ...
        PREF_CUSTOM_JAVA_ARGS = DEFAULT_PREF.getString("javaArgs", "-XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=20 -XX:+UnlockExperimentalVMOptions -XX:+AlwaysPreTouch");
    ...
    private static int findBestRAMAllocation(Context ctx){
        ...
        if (is32BitsDevice()) return 800;

        if (deviceRam < 3064) return 1024;
        if (deviceRam < 4096) return 1536;
        if (deviceRam < 6144) return 2048;
        return 2560; // Optimal default RAM allocation for 64 bits modern MC
    }
    ...
    private static int findBestResolution(Context context, boolean isDevicePowerful) {
        ...
        int targetSide = isDevicePowerful ? 800 : 720;
